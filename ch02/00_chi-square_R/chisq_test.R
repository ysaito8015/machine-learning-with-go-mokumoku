...
{
    # depase() : Turn unevaluated expressions into character strings.  
    #
    # substitute()
    ...
    if (is.matrix(x)) {
        ...
        nr <- as.integer(nrow(x)) # nRow
        nc <- as.integer(ncol(x)) # nColumn
        if (is.na(nr) || is.na(nc) || is.na(nr * nc))
            stop("invalid nrow(x) or ncol(x)", domain = NA)
        sr <- rowSums(x) # sum of row
        sc <- colSums(x) # sum of column
